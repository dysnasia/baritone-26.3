...
- `mine diamond_ore iron_ore` to mine diamond ore or iron ore (turn on the setting `legitMine` to only mine ores that it can actually see. It will explore randomly around y=11 until it finds them.) A block count can also be given first, for example `mine 64 diamond_ore`, which stops after 64 diamond ore blocks have been broken, no matter how many items they drop.
